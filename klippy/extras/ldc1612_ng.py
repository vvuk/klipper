...
class LDC1612_ng:
    def __init__(self, config, sensor_type=None):
        ...
        if config.get('intb_pin', None) is not None:
            ...
            # TODO: how does intb_pin=%s work? it's %c on the other end
            mcu.add_config_cmd(
                "config_ldc1612 oid=%d i2c_oid=%d intb_pin=%s"
                % (oid, self._i2c.get_oid(), pin_params['pin']))
        else:
            mcu.add_config_cmd("config_ldc1612 oid=%d i2c_oid=%d intb_pin=%d"
                               % (oid, self._i2c.get_oid()), -1)
    ...
    def _build_config(self):
        ...
        self._ldc1612_ng_finish_home_cmd = self._mcu.lookup_query_command(
             ...
             "ldc1612_ng_finish_home_reply oid=%c homing=%c trigger_clock=%u tap_end_clock=%u tap_amount=%u",
             oid=self._oid, cq=cmdqueue)
    ...
    def finish_home(self):
        ...
        trigger_time = self._clock32_to_print_time(reply['trigger_clock'])
        tap_end_time = self._clock32_to_print_time(reply['tap_end_clock'])
        tap_amount = reply['tap_amount']

        return LDC1612_ng_homing_result(active, trigger_time, tap_end_time, tap_amount)
