...
command_config_ldc1612_ng(uint32_t *args)
{
    ...
    if (intb_pin != 0xffffffff) {
        ld->intb_pin = gpio_in_setup(intb_pin, 1);
        ld->flags = LDC_HAVE_INTB;
    }
}
DECL_COMMAND(command_config_ldc1612_ng, "config_ldc1612_ng oid=%c i2c_oid=%c intb_pin=%d");
...
{
    struct ldc1612_ng *ld = oid_lookup(args[0], command_config_ldc1612_ng);
    struct ldc1612_homing *lh = &ld->homing;
    ...
    if (lh->homing_flags & LH_CAN_TRIGGER) {
        notify_trigger(ld, 0, other_reason_base);
    ...
    if (tap_threshold) {
        lh->homing_flags |= LH_WANT_TAP;
    }

    dprint("ZZZ setup sf=%u tf=%u tap=%d", start_freq, trigger_freq, tap_threshold);
}
...
ldc1612_ng_update(struct ldc1612_ng *ld, uint8_t oid)
{
    ...
    if (ld->homing_flags & LH_CAN_TRIGGER) {
        ld1612_ng_check_home(ld, data, time);
    }
...
ldc1612_ng_check_home(struct ldc1612_ng* ld, uint32_t data, uint32_t time)
{
    ...
    if (SAMPLE_ERR(data)) {
        ...
        notify_trigger(lh, 0, ld->other_reason_base + REASON_ERROR_SENSOR);
        return;
    }
    ...
    if (homing_flags & LH_AWAIT_HOMING) {
        ...
        if (ld->safe_start_time != 0 && timer_is_before(time, ld->safe_start_time)) {
            ...
            notify_trigger(lh, 0, lh->other_reason_base + REASON_ERROR_TOO_EARLY);
            return;
        }
    ...
    if (!is_tap) {
        ...
        if (data > ld->homing_trigger_freq) {
            notify_trigger(lh, time, ld->success_reason);
            ld->trigger_time = time;
            dprint("ZZZ home t=%u a=%u (f=%u)", time, wma, data);
        }
    } else {
        if (ld->tap_accum > ld->tap_threshold) {
            // Note: we notify with the time the tap started, not the current time
            notify_trigger(lh, ld->tap_start_time, ld->success_reason);
